...
class EVAutoRepairApp(ctk.CTk):
    def __init__(self, db: DB):
        ...
        # main layout: top bar + left menu + right content
    ...
    def build_dashboard_page(self, parent: ctk.CTkFrame):
        ...
        # create four large rectangular stat cards for monthly overview
        ...
        # create four large rectangular stat cards
        ...
        # small refresh button under cards
        refresh_btn = ctk.CTkButton(parent, text="Refresh", command=self.refresh_dashboard)
        refresh_btn.pack(anchor="w", pady=8)

        # removed the table of today's repair orders to show only the summary stats

    def refresh_dashboard(self):
        stats = self.db.get_today_stats()
        # update large values on cards
        # update large values on cards (today)
        try:
    ...
    def update_service_price(self):
        ...
        # Optionally, add part prices if you want to total them (requires price info per part)
    ...
    def add_diagnosis(self):
        ...
        def do_ok():
            ...
            # Append diagnosis into the selected vehicle's `problem` field in DB
    ...
    def refresh_orders_table(self):
        ...
        # Aggregate orders by client+vehicle so one client row shows multiple diagnoses/services
        aggr = {}
        aggr_oids = {}
        for r in rows:
